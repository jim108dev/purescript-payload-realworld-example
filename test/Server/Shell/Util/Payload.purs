module Test.Server.Shell.Util.Payload where
...
withServer apiSpec api_ aff = do
  let
    opts = Payload.defaultOpts { logLevel = Payload.LogError, port = 3000 }
...
-- | Function did not exist in the original source
post_ :: String -> String -> Headers -> Maybe String -> Aff ApiResponse
post_ host path headers reqBody = AX.request req >>= decodeResponse
  where
  req =
    AX.defaultRequest
      { content = RequestBody.String <$> reqBody
      , method = Left POST
      , url = host <> "/" <> path
      , responseFormat = ResponseFormat.string
      , headers = (\(Tuple name val) -> RequestHeader name val) <$> Headers.toUnfoldable headers
...
put host path reqBody = do
  let
    body = Just $ RequestBody.String reqBody
  result <- AX.put ResponseFormat.string (host <> "/" <> path) body
  decodeResponse result

-- | Function did not exist in the original source
put_ :: String -> String -> Headers -> Maybe String -> Aff ApiResponse
put_ host path headers reqBody = AX.request req >>= decodeResponse
  where
  req =
    AX.defaultRequest
      { content = RequestBody.String <$> reqBody
...
-- | Function did not exist in the original source
delete_ :: String -> String -> Headers -> Maybe String -> Aff ApiResponse
delete_ host path headers reqBody = AX.request req >>= decodeResponse
  where
  req =
    AX.defaultRequest
      { content = RequestBody.String <$> reqBody
...
respMatches expected received = Assert.equal expected { status: received.status, body: received.body }

respMatchesStatus :: Int -> ApiResponse -> Test
respMatchesStatus expected received = Assert.equal expected received.status

-- | The original function just compares string bodies, this function parses 
-- | first and then compares. 
respMatchesJson :: { status :: Int, body :: String } -> ApiResponse -> Test
respMatchesJson expected received = do
  Assert.equal expected.status received.status
  assertBodyEquals expected.body received.body

assertBodyEquals :: String -> String -> Aff Unit
assertBodyEquals s1 s2 = do
  result <-
    runExceptT do
      j1 <- except $ jsonParser s1
      j2 <- except $ jsonParser s2
      pure $ j1 == j2
  case result of
    Left err -> throwError (error $ "Could not parse body: " <> show err)
    Right value -> Assert.assert ("Expected " <> s1 <> ", got " <> s2) value
